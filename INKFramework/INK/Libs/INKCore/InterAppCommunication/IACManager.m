...
- (BOOL)handleOpenURL:(NSURL*)url {
    ...
    if ([url.host isEqualToString:kXCUHost]) {
        ...
        [pairs enumerateObjectsUsingBlock:^(NSString *pair, NSUInteger idx, BOOL *stop) {
            ...
            if ([comps count] == 2) {
                [parameters setObject:[comps[1] stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding] forKey:comps[0]];
            }
        }];
        ...
        NSLog(@"LDCDL action: %@", action);
        // Lets see if there is somebody that handles this action
        if (actions[action][0] || [self.delegate supportsIACAction:action]) {
            ...
            if (actions[action][0]) {
                NSMutableArray *handlers = actions[action];
                for (IACActionHandlerBlock actionHandler in handlers) {
...
- (void)sendIACRequest:(IACRequest*)request {
    ...
    NSString *finalUrl = [NSString stringWithFormat:@"%@://%@/%@?", request.client.URLScheme, kXCUHost, request.action];
    finalUrl = [finalUrl stringByAppendingURLParams:request.parameters];
    finalUrl = [finalUrl stringByAppendingURLParams:@{kXCUSource: [self localizedAppName]}];
    ...
    if (self.callbackURLScheme) {
        ...
        finalUrl = [finalUrl stringByAppendingURLParams:xcu_params];
...
- (void)handleAction:(NSString*)action withBlock:(IACActionHandlerBlock)handler {
    ...
    */
    //TODO: Overwriting so only one handler
